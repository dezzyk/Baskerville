...
glm::vec2 Widget::calcDrawPos(entt::entity entity, entt::registry &reg) {

    Widget& widget = reg.get<Widget>(entity);
    glm::vec2 draw_size, draw_pos, parent_draw_size;
    draw_size = calcDrawSize(entity, reg);
    ...
    if(widget.parent.has_value()) {
        parent_draw_size = calcDrawSize(widget.parent.value(), reg);
        draw_pos = calcDrawPos(widget.parent.value(), reg);
    } else {
...
glm::vec2 Widget::calcDrawSize(entt::entity entity, entt::registry &reg) {
