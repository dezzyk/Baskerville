...
void Test::draw(Draw::CallQueue& draw_buffer) {
    ...
    model = glm::scale(model, glm::vec3(m_font->calcScale(24) * Platform::getGlobalScaler(), m_font->calcScale(24)* Platform::getGlobalScaler(), 0.0f));

    Draw::Call new_draw;
    new_draw.context = &m_draw_context;
    new_draw.shader = m_shader;
    new_draw.count = 6 * boxes.size();
